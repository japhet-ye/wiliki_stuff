	...
	if (n>0) {
		switch(man_cmd) {
			...
			case 'u': /* Upload a file to a host */
				...
				break;
			case 't'://Start discovery of tree
				scanf(man_msg, "%d %s", &dst, name);
				new_packet = (struct packet *)
					malloc(sizeof(struct packet));
				for(i=0; name[i] !='\0'; ++i){
				...
				new_packet->src = (char) host_id;
				new_packet->dst = (char) dst;
				new_packet->type = (char) PKT_TREE_DISCOVERY;
				new_packet->RootID = 500; //Set arbitrarily high value
				new_packet->RootDist = -1; //Set so that when switch recieves, it will be 0
				new_packet->SenderType = 'H'; //Sender type is a Host
				new_packet->SenderChild = 'N'; //Host is not a child

				//Create job to send tree packet out of all ports
				new_job = (struct host_job *)
					malloc(sizeof(struct host_job));
				new_job->type = JOB_SEND_PKT_ALL_PORTS;
				new_job->file_upload_dst = dst;
