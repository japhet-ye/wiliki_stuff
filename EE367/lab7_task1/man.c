...
while(1) {
	...
	printf("   (t) Initiate Root Finding Sequence for Tree \n");
...
void tree_send(struct man_port_at_man *curr_host)
{
printf("Sending Tree Packet out all ports now \n");
char msg[MAN_MSG_LENGTH];
int n;
n = sprintf(msg, "t");
write(curr_host->send_fd, msg, n);
   ...
   /* Execute the command */
	switch(cmd)
	{
		...
		case 'u': /* Upload a file from the current host
			...
			break;
		case 't' : /*Starts discovery of trees and root */
			tree_send(curr_host);
